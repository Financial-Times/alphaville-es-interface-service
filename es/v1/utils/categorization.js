...
exports.isFirstFT = function (article) {
	...
	if (article) {
		if (article.isFirstFT === true) {
			isFirstFT = true;
		} else if (filterMetadata(article.annotations, { prefLabel: 'First FT' }).length > 0 || filterMetadata(article.metadata, { prefLabel: 'FirstFT' }).length > 0) {
			isFirstFT = true;
		}
	}
	return isFirstFT;
...
exports.isSeriesArticle = function (article) {
	...
	if (article) {
		...
		} else if (filterMetadata(article.annotations, {directType:'http://www.ft.com/ontology/AlphavilleSeries'}).length > 0) {
			isSeriesArticle = true;
		}
	}
