...
exports.processArticle = function (article) {
	return new Promise((resolve) => {
		if (article) {
			const authorsAnnotations = filterAnnotations(article.annotations, item => item.type === 'PERSON' && item.predicate === 'http://www.ft.com/ontology/annotation/hasAuthor');
			const authors = [];

			if (authorsAnnotations.length) {
				authorsAnnotations.forEach(author => {
					const obj = {};
					obj.concept = author;
