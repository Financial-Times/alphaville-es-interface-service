...
function populateContent (wpPath, article, isMock, withContent) {
		...
		return wpApi.init().then((init) => {
			if (init.data.status === 'closed' && withContent) {
				return Promise.all([wpApi.transcriptJson(), commentsApi.getAllComments({
						...
						url: process.env.APP_URL + article._source.av2WebUrl,
						order: 'ascending'
					})])
				.then((response) => {
					const content = response[0];
	...
	return fetch().then((response) => {
		...
		if (initResponse.data.status === 'inprogress') {
			article.isLive = true;
			article._source.isLive = true;
		} else {
			article.isLive = false;
			article._source.isLive = false;
		}


		article._source.subheading = initResponse.data.post_excerpt;
		article._source.openingHTML = `<p>${initResponse.data.post_excerpt}</p>`;
			...
			if (!article.isLive && jsonResponse && jsonResponse.success) {
				...
				if (bodyHTML) {
					...
					$('img').each(function () {
						...
						if (image.attr('class').indexOf('emoticon') > -1 && srcMatch) {
							image.replaceWith(`
					...
					article._source.bodyHTML = '<div class="webchat-closed-content">' + bodyHTML + '</div>';
				}
			}
		}
...
exports.processArticle = function (article, withContent) {
	return new Promise((resolve) => {
		if (article && article._source && article._source.webUrl && article._source.webUrl.indexOf('marketslive') !== -1) {


			article.isMarketsLive = true;
			article._source.isMarketsLive = true;

			populateContent(article._source.av2WebUrl, article, false, withContent).then(article => {
				resolve(article);
			}).catch((err) => {
				if (process.env.ML_TRANSCRIPT_MOCK_URL) {
					...
					console.log("Error fetching MarketsLive data for " + article._source.av2WebUrl, err, err.stack);
