...
exports.processArticle = function (article) {
	return new Promise((resolve) => {
		if (article._source) {
			const authorsMetadata = filterMetadata(article._source.metadata, { taxonomy: 'authors' });
			const authors = [];
			...
			if (authorsMetadata.length) {
				...
				if (article._source.byline) {
					const bylineAuthors = article._source.byline.split(',');
			...
			article._source.authors = authors;
		}
